...
if (jikan.broadcast != null) {
        computeNextAiringFromBroadcast(jikan.broadcast)?.let { (episodeTime) ->                        this.anime?.nextAiringEpisodeTime = episodeTime                        nextAiringTime = episodeTime                    }
}
try {
...
if (!nextAiring.isAfter(now)) {
        nextAiring = now.with(java.time.temporal.TemporalAdjusters.next(dayOfWeek))                .withHour(hour).withMinute(minute).withSecond(0).withNano(0)
        }
