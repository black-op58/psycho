...
private const val REGEX_EPISODE =        "(episode|episodio|ep|e)${REGEX_ITEM}\\(?\\s*(sub|subbed|dub|dubbed)*\\s*\\)?\\s*"    
...
if (episodeMatcher.group(2) != null) {                episodeMatcher.group(2)?.toFloat()
} else {
...
return if (matcher.find()) {            matcher.group(2)?.toFloat()
} else {
