...
override fun onViewCreated(view: View, savedInstanceState: Bundle?) {        super.onViewCreated(view, savedInstanceState)        userId = arguments?.getInt("userId", -1) ?: -1
...
} else {            binding.profileFavAnimeContainer.visibility = View.VISIBLE            binding.profileFavAnimeRecyclerView.apply {                adapter = MediaAdaptor(0, ArrayList(favAnime), requireActivity())                layoutManager = LinearLayoutManager(
                    requireContext(), LinearLayoutManager.HORIZONTAL, false                )                enableDpadNavigation()            }
}
if (favManga.isEmpty()) {            binding.profileFavMangaContainer.visibility = View.GONE
} else {            binding.profileFavMangaContainer.visibility = View.VISIBLE            binding.profileFavMangaRecyclerView.apply {                adapter = MediaAdaptor(0, ArrayList(favManga), requireActivity())                layoutManager = LinearLayoutManager(
                    requireContext(), LinearLayoutManager.HORIZONTAL, false                )                enableDpadNavigation()            }
}
...
} else {            binding.profileFavCharacterContainer.visibility = View.VISIBLE            binding.profileFavCharacterRecyclerView.apply {                adapter = CharacterAdapter(favCharacters, requireActivity())                layoutManager = LinearLayoutManager(
                    requireContext(), LinearLayoutManager.HORIZONTAL, false                )                enableDpadNavigation()            }}
}
