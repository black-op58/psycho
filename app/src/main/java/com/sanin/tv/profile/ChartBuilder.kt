...
for (i in statData.indices) {                statValues.add(arrayOf(names[i], statData[i], statData[i]))            }
...
if (names[i] is Number) {                    element.x(names[i] as Number)
        element.dataLabels(
...
private fun getToolTipFunction(            chartType: ChartType,            type: String,            typeName: String,            chartSize: Int        ): String {
    ...
    if (chartSize == 1) {                        """        function () {
    return '$type: ' +        this.x +        '<br/> ' +        ' $typeName ' +        this.y        }            """.trimIndent()                    } else {                        """function() {    let wholeContentStr = '<span style=\"' + 'color:gray
font-size:13px\"' + '>◉${type}: ' + this.x + '</span><br/>'
if (this.points) {    let length = this.points.length
for (let i = 0
i < length
i++) {        let thisPoint = this.points[i]
let yValue = thisPoint.y
if (yValue != 0) {            let spanStyleStartStr = '<span style=\"' + 'color: ' + thisPoint.color + '
font-size:13px\"' + '>◉ '
