...
companion object {
    ...
    var query = "{"            mediaIds.forEachIndexed { index, mediaId ->                query += """                media$index: Media(id: $mediaId) {                    coverImage {                        medium                        color                    }                    id                    title {                        romaji                    }                }            """.trimIndent()            }            query += "}"            return query        }        suspend 
...
val mediaMap = mutableMapOf<Int, ReturnedData>()                    mediaResponse.data.forEach { (_, mediaItem) ->                        mediaMap[mediaItem.id] = ReturnedData(                            mediaItem.title.romaji,                            mediaItem.coverImage.medium,                            mediaItem.coverImage.color                        )                    }                    mediaMap                }            } catch (e: Exception) {
    val errorMap = mutableMapOf<Int, ReturnedData>()                ids.forEach { errorMap[it] = ReturnedData("Unknown", "", "#222222") }                errorMap            }        }
