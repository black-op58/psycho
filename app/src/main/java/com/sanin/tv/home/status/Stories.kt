...
if (::storiesListener.isInitialized) {
        storyIndex = 1            storiesListener.onStoriesEnd()
        resetProgressBar(storyIndex)
         }
}
...
if (::storiesListener.isInitialized) {
        storyIndex = 1            storiesListener.onStoriesStart()
        resetProgressBar(storyIndex)
         }
}
...
if (!hostActivity.isFinishing &&                            !hostActivity.isDestroyed &&                            hostActivity.hasWindowFocus()                        ) {
        resume()                        }}}
...
when (event.action) {
        ...
        startX = event.x                startY = event.y                startClickTime = Calendar.getInstance().timeInMillis                pause();
        isLongPress = false
            }
