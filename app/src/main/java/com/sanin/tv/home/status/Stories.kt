...
import com.sanin.tv.databinding.FragmentStatusBinding
...
import com.sanin.tv.profile.activity.RepliesBottomDialog
import com.sanin.tv.settings.saving.PrefManager
import com.sanin.tv.settings.saving.PrefName
import com.sanin.tv.snackString
import com.sanin.tv.util.AniMarkdown
import com.sanin.tv.util.AnilistLinkParser
...
class Stories 
@JvmOverloads constructor(    context: Context, attrs: AttributeSet? = null, defStyleAttr: Int = 0) : ConstraintLayout(context, attrs, defStyleAttr), View.OnTouchListener {
...
} else {                    constraintSet.connect(                        getId("story${counter}"),                        ConstraintSet.END,                        LayoutParams.PARENT_ID,                        ConstraintSet.END                    )                    constraintSet.connect(                        getId("story${counter}"),                        ConstraintSet.TOP,                        LayoutParams.PARENT_ID,                        ConstraintSet.TOP                    )                    constraintSet.connect(                        getId("story${counter}"),                        ConstraintSet.START,                        LayoutParams.PARENT_ID,                        ConstraintSet.START                    )                }            }            counter--        }        constraintSet.applyTo(binding.progressBarContainer)        startShowContent()    }
...
} else {                // on stories end                binding.androidStoriesLoadingView.visibility = View.GONE                onStoriesCompleted()            }        }        timer.setOnPercentTickListener {            progressBar.progress = it        }        loadStory(activityList[storyIndex - 1])    }
...
return        }        userClicked = true        timer.cancel()
if (storyIndex <= activityList.size) storyIndex += 1        showStory()    }

private fun leftPanelTouch() {        Logger.log("leftPanelTouch: $storyIndex")
if (storyIndex == 1) {            onStoriesPrevious()
return        }        userClicked = true        timer.cancel()        resetProgressBar(storyIndex)
...
val newList = set.sorted().takeLast(200).toSet()        PrefManager.setCustomVal(key, newList)        binding.statusUserAvatar.loadImage(story.user?.avatar?.large)        binding.statusUserName.text = story.user?.name        binding.statusUserTime.text = ActivityItemBuilder.getDateTime(story.createdAt)        binding.statusUserContainer.setOnClickListener {            ContextCompat.startActivity(                context,                Intent(context, ProfileActivity::class.java).putExtra("userId", story.userId),                null            )        }        binding.textActivity.setOnTouchListener { v, event ->            onTouchView(v, event, true)            v.onTouchEvent(event)        }        binding.textActivityContainer.setOnTouchListener { v, event ->            onTouchView(v, event, true)            v.onTouchEvent(event)        }
...
} else {                            it.toString()                        }                    }                } ${story.progress ?: story.media?.title?.userPreferred} " +
...
} else {                            ""                        }                binding.infoText.text = text
val bannerAnimations: Boolean = PrefManager.getVal(PrefName.BannerAnimations)                blurImage(
if (bannerAnimations) binding.contentImageViewKen else binding.contentImageView,                    story.media?.bannerImage ?: story.media?.coverImage?.extraLarge                )                binding.coverImage.loadImage(story.media?.coverImage?.extraLarge)                binding.coverImage.setOnClickListener {                    ContextCompat.startActivity(                        context,                        Intent(context, MediaDetailsActivity::class.java).putExtra(                            "mediaId",                            story.media?.id                        ),                        ActivityOptionsCompat.makeSceneTransitionAnimation(                            (it.context as FragmentActivity),                            binding.coverImage,                            ViewCompat.getTransitionName(binding.coverImage)!!                        ).toBundle()                    )                }            }            "TextActivity" -> {                visible(false)
...
val markwon = buildMarkwon(context, false)                    markwon.setMarkdown(binding.textActivity, cleanedHtml)                    addLinkPreviews(anilistLinks)                }            }            "MessageActivity" -> {                visible(false)
if (!(context as android.app.Activity).isDestroyed) {
...
if (!hostActivity.isFinishing &&                            !hostActivity.isDestroyed &&                            hostActivity.hasWindowFocus()                        ) {                            resume()                        }                    }                }            }.show(hostActivity.supportFragmentManager, "replies")        }        binding.activityLike.setColorFilter(if (story.isLiked == true) likeColor else notLikeColor)        binding.activityLikeCount.text = story.likeCount.toString()        binding.activityLikeContainer.setOnClickListener {            like()        }        binding.activityLikeContainer.setOnLongClickListener {            UsersDialogFragment().apply {                userList(userList)                show((it.context as FragmentActivity).supportFragmentManager, "dialog")            }            true        }        binding.androidStoriesLoadingView.visibility = GONE        timer.start()    }
...
} else {                        story.likeCount = story.likeCount?.plus(1)                    }                    binding.activityLikeCount.text = (story.likeCount ?: 0).toString()                    story.isLiked = !story.isLiked!!                    binding.activityLike.setColorFilter(if (story.isLiked == true) likeColor else notLikeColor)
...
return        }        binding.linkPreviewContainer.visibility = VISIBLE
...
} else {                            previewView.visibility = View.GONE                        }                                                binding.linkPreviewContainer.addView(previewView)                    }                }            }        }    }
...
when (event.action) {            MotionEvent.ACTION_DOWN -> {                startX = event.x                startY = event.y                startClickTime = Calendar.getInstance().timeInMillis                pause()                isLongPress = false            }            MotionEvent.ACTION_MOVE -> {
    val deltaX = event.x - startX
val deltaY = event.y - startY
if (!isLongPress && (abs(deltaX) > swipeThreshold || abs(deltaY) > swipeThreshold)) {                    isLongPress = true                }            }            MotionEvent.ACTION_UP -> {
    val clickDuration = Calendar.getInstance().timeInMillis - startClickTime
if (isText) {
if (clickDuration < maxClickDuration && !isLongPress) {
if (event.x < leftQuarter) {                            leftPanelTouch()
} else if (event.x > rightQuarter) {                            rightPanelTouch()
} else {                            resume()                        }
} else {                        resume()                    }
} else {
if (clickDuration < maxClickDuration && !isLongPress) {
if (event.x < leftHalf) {                            leftPanelTouch()
} else {                            rightPanelTouch()                        }
} else {                        resume()                    }                }
