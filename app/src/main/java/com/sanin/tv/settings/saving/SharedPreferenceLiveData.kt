...
class SharedPreferenceStringSetLiveData(    sharedPrefs: SharedPreferences,    key: String,    defValue: Set<String>) :    SharedPreferenceLiveData<Set<String>>(sharedPrefs, key, defValue) {
    override fun getValueFromPreferences(key: String, defValue: Set<String>): Set<String> =        sharedPrefs.getStringSet(key, defValue)?.toSet() ?: defValue}

@Suppress("UNCHECKED_CAST")
class SharedPreferenceClassLiveData<T>(    sharedPrefs: SharedPreferences,    key: String,    defValue: T) : SharedPreferenceLiveData<T>(sharedPrefs, key, defValue) {
...
val obj = ois.readObject() as T                obj
        ...
        Logger.log("Serialized data is null (key: $key)")                defValue
            }
}
        catch (e: java.io.InvalidClassException) {
        Logger.log(e)
try {
        sharedPrefs.edit().remove(key).apply()                defValue
            }
        catch (e: Exception) {
        Logger.log(e)                defValue
            }
}
        catch (e: Exception) {
        Logger.log(e)            defValue}
