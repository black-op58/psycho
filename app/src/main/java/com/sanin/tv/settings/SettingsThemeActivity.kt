...
val context = this        binding = ActivitySettingsThemeBinding.inflate(layoutInflater)        setContentView(binding.root)        binding.apply {            settingsThemeLayout.updateLayoutParams<ViewGroup.MarginLayoutParams> {                topMargin = statusBarHeight                bottomMargin = navBarHeight            }            onBackPressedDispatcher.addCallback(context) {
if (reload) {
...
try {                        startActivity(Intent().setComponent(component))                    } catch (e: Exception) {                        startActivity(mainIntent)                    }                    finishAndRemoveTask()                    reload = false
} else {                    finish()                }            }            themeSettingsBack.setOnClickListener {                onBackPressedDispatcher.onBackPressed()            }

var previous: View = when (PrefManager.getVal<Int>(PrefName.DarkMode)) {                0 -> settingsUiAuto                1 -> settingsUiLight                2 -> settingsUiDark
else -> settingsUiAuto            }            previous.alpha = 1f
fun uiTheme(mode: Int, current: View) {                previous.alpha = 0.33f                previous = current                current.alpha = 1f                PrefManager.setVal(PrefName.DarkMode, mode)                reload()            }            settingsUiAuto.setOnClickListener {                uiTheme(0, it)            }            settingsUiLight.setOnClickListener {                PrefManager.setVal(PrefName.OledMode, 0)                uiTheme(1, it)            }            settingsUiDark.setOnClickListener {                uiTheme(2, it)            }

val themeString: String = PrefManager.getVal(PrefName.Theme)            
val themeText = themeString.substring(0, 1) + themeString.substring(1).lowercase()            themeSwitcher.apply {                setText(themeText)                setAdapter(                    ArrayAdapter(context,                        R.layout.item_dropdown,                        ThemeManager.Companion.Theme.entries.map {                            it.theme.substring(                                0,                                1                            ) + it.theme.substring(1).lowercase()                        })                )                setOnItemClickListener { _, _, i, _ ->                    PrefManager.setVal(                        PrefName.Theme,                        ThemeManager.Companion.Theme.entries[i].theme                    )                    clearFocus()                    reload()                }            }            settingsRecyclerView.adapter = SettingsAdapter(                arrayListOf(                    Settings(                        type = 1,                        name = getString(R.string.oled_background_mode),                        desc = listOf(                            getString(R.string.oled_mode_off),                            getString(R.string.oled_mode_pure),                            getString(R.string.oled_mode_glow),                            getString(R.string.oled_mode_gradient),                            getString(R.string.oled_mode_vignette)                        ).getOrElse(PrefManager.getVal<Int>(PrefName.OledMode)) { getString(R.string.oled_mode_off) },                        icon = R.drawable.ic_round_brightness_4_24,                        onClick = { b ->                            val modeNames = arrayOf(                                "${getString(R.string.oled_mode_off)}\n${getString(R.string.oled_mode_off_desc)}",                                "${getString(R.string.oled_mode_pure)}\n${getString(R.string.oled_mode_pure_desc)}",                                "${getString(R.string.oled_mode_glow)}\n${getString(R.string.oled_mode_glow_desc)}",                                "${getString(R.string.oled_mode_gradient)}\n${getString(R.string.oled_mode_gradient_desc)}",                                "${getString(R.string.oled_mode_vignette)}\n${getString(R.string.oled_mode_vignette_desc)}"                            )                            val current = PrefManager.getVal<Int>(PrefName.OledMode)                            customAlertDialog().apply {                                setTitle(R.string.oled_background_mode)                                singleChoiceItems(modeNames, current) { idx ->                                    PrefManager.setVal(PrefName.OledMode, idx)                                    b.settingsDesc.text = listOf(                                        getString(R.string.oled_mode_off),                                        getString(R.string.oled_mode_pure),                                        getString(R.string.oled_mode_glow),                                        getString(R.string.oled_mode_gradient),                                        getString(R.string.oled_mode_vignette)                                    ).getOrElse(idx) { getString(R.string.oled_mode_off) }                                    reload()                                }                                setNegButton(R.string.cancel)                                show()                            }                        }                    ),                    Settings(                        type = 1,                        name = getString(R.string.gradient_direction),                        desc = listOf(                            getString(R.string.gradient_dir_top_bottom),                            getString(R.string.gradient_dir_bottom_top),                            getString(R.string.gradient_dir_left_right),                            getString(R.string.gradient_dir_right_left)                        ).getOrElse(PrefManager.getVal<Int>(PrefName.GradientDirection)) { getString(R.string.gradient_dir_top_bottom) },                        icon = R.drawable.ic_round_brightness_4_24,                        onClick = { b ->                            val dirNames = arrayOf(                                getString(R.string.gradient_dir_top_bottom),                                getString(R.string.gradient_dir_bottom_top),                                getString(R.string.gradient_dir_left_right),                                getString(R.string.gradient_dir_right_left)                            )                            val current = PrefManager.getVal<Int>(PrefName.GradientDirection)                            customAlertDialog().apply {                                setTitle(R.string.gradient_direction)                                singleChoiceItems(dirNames, current) { idx ->                                    PrefManager.setVal(PrefName.GradientDirection, idx)                                    b.settingsDesc.text = dirNames.getOrElse(idx) { dirNames[0] }                                    reload()                                }                                setNegButton(R.string.cancel)                                show()                            }                        }                    ),                    Settings(                        type = 2,                        name = getString(R.string.swap_colors),                        desc = getString(R.string.swap_colors_desc),                        icon = R.drawable.ic_round_brightness_4_24,                        isChecked = PrefManager.getVal(PrefName.SwapColors),                        switch = { isChecked, _ -> PrefManager.setVal(PrefName.SwapColors, isChecked); reload() }                    ),                    Settings(                        type = 2,                        name = getString(R.string.use_material3),                        desc = getString(R.string.use_material3_desc),                        icon = R.drawable.ic_round_new_releases_24,                        isChecked = PrefManager.getVal(PrefName.UseMaterial3),                        switch = { isChecked, _ -> PrefManager.setVal(PrefName.UseMaterial3, isChecked); reload() }                    ),                    Settings(                        type = 1,                        name = getString(R.string.blend_level),                        desc = "${PrefManager.getVal<Int>(PrefName.BlendLevel)}",                        icon = R.drawable.ic_round_brightness_4_24,                        onClick = { b -> val opts = (0..20).map { "$it" }.toTypedArray(); val cur = PrefManager.getVal<Int>(PrefName.BlendLevel); customAlertDialog().apply { setTitle(R.string.blend_level); singleChoiceItems(opts, cur) { idx -> PrefManager.setVal(PrefName.BlendLevel, idx); b.settingsDesc.text = "$idx" }; setNegButton(R.string.cancel); show() } }                    ),                    Settings(                        type = 2,                        name = getString(R.string.use_material_you),                        desc = getString(R.string.use_material_you_desc),                        icon = R.drawable.ic_round_new_releases_24,                        isChecked = PrefManager.getVal(PrefName.UseMaterialYou),                        switch = { isChecked, _ ->                            PrefManager.setVal(PrefName.UseMaterialYou, isChecked)
...
val tag = "colorPicker"                            CustomColorDialog().title(R.string.custom_theme)                                .colorPreset(originalColor)                                .colors(context, SimpleColorDialog.MATERIAL_COLOR_PALLET)                                .allowCustom(true).showOutline(0x46000000).gridNumColumn(5)                                .choiceMode(SimpleColorDialog.SINGLE_CHOICE).neg()                                .show(context, tag)                        },                        isVisible = Build.VERSION.SDK_INT > Build.VERSION_CODES.R                    ),
                    Settings(
                        ...
                        onClick = { b ->
                            val opts = (0..4).map { String.format("%.1f×", 0.5f + it * 0.375f) }.toTypedArray()
                            val vals = (0..4).map { 0.5f + it * 0.375f }
