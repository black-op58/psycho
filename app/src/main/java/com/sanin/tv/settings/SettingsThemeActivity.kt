...
val current = PrefManager.getVal<Int>(PrefName.GradientDirection)                            customAlertDialog().apply {                                setTitle(R.string.gradient_direction)                                singleChoiceItems(dirNames, current) { idx ->                                    PrefManager.setVal(PrefName.GradientDirection, idx)                                    b.settingsDesc.text = dirNames.getOrElse(idx) { dirNames[0]}
reload()}
setNegButton(R.string.cancel)                                show()}}
