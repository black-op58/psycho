...
fun routeAudioToSpeaker() {        audioManager.requestAudioFocus(            null,            AudioManager.STREAM_MUSIC,            AudioManager.AUDIOFOCUS_GAIN        )        audioManager.mode = AudioManager.MODE_IN_COMMUNICATION        audioManager.isSpeakerphoneOn = true    }
...
fun playAudio(audio: Int) {        mediaPlayer?.release()        mediaPlayer = MediaPlayer.create(context, audio)
        mediaPlayer?.setOnCompletionListener {
            setVolume(oldVolume)            audioManager.abandonAudioFocus(null)
            it.release()
        }
...
companion object {
    ...
    val audioHelper = AudioHelper(context)            audioHelper.routeAudioToSpeaker()
