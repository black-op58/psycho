...
override fun onChildViewDetachedFromWindow(view: View) {}        })        setOnKeyListener { _, keyCode, event ->
...
if (first > 0) smoothScrollToPosition(first - 1)                    true                }                KeyEvent.KEYCODE_DPAD_DOWN -> {
    val last = lm.findLastVisibleItemPosition()
if (last < (adapter?.itemCount ?: 0) - 1) smoothScrollToPosition(last + 1)                    true                }                KeyEvent.KEYCODE_DPAD_LEFT -> {
if (lm.orientation == LinearLayoutManager.HORIZONTAL) {
    val first = lm.findFirstVisibleItemPosition()
if (first > 0) smoothScrollToPosition(first - 1)                        true
} else false                }                KeyEvent.KEYCODE_DPAD_RIGHT -> {
if (lm.orientation == LinearLayoutManager.HORIZONTAL) {
    val last = lm.findLastVisibleItemPosition()
if (last < (adapter?.itemCount ?: 0) - 1) smoothScrollToPosition(last + 1)                        true
} else false                }                KeyEvent.KEYCODE_DPAD_CENTER, KeyEvent.KEYCODE_ENTER -> {
...
true } else false            }            KeyEvent.KEYCODE_DPAD_RIGHT -> {
