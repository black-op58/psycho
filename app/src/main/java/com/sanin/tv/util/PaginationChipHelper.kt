...
val endChapterString = MediaNameAdapter.findChapterNumber(names[last - 1])?.let { "Ch.%.1f".format(it) } ?: names[last - 1]            chip.text = "$startChapterString - $endChapterString"            chip.setTextColor(                ContextCompat.getColorStateList(                    context,                    R.color.chip_text_color                )            )            chip.setOnClickListener {
                selectedChip()                onChipClicked(position, limit * position, last - 1)
            }
