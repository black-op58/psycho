...
fun characterInformation(includeMediaInfo: Boolean) = """    id    name {      
    ...
    if (includeMediaInfo) """    media(page: 0,sort:[POPULARITY_DESC,SCORE_DESC]) {      $standardPageInformation      edges {
        ...
        characterRole
        node {
        id          idMal          isAdult          status          chapters          episodes          nextAiringEpisode {
        episode }
        type
        meanScore          isFavourite          format          bannerImage          countryOfOrigin          coverImage {
        large }
        title {
            english              romaji              userPreferred          }
        mediaListEntry {
        progress              progressVolumes              
private              score(format: POINT_100);
        status
        }
...
fun studioInformation(page: Int, perPage: Int) = """    id    name    isFavourite    favourites    media(page: $page, sort:START_DATE_DESC, perPage: $perPage) {      
        ...
        year }
        isFavourite
        format          bannerImage          countryOfOrigin          coverImage {
        large }
        title {
            english              romaji              userPreferred          }
        mediaListEntry {
...
private        siteUrl        createdAt        updatedAt        user {          
    ...
    source    duration    season    seasonYear    startDate {
        year      month      day    }
    endDate {
        year      month      day    }
    studios(isMain: true) {
    ...
    characters(sort: [ROLE, FAVOURITES_DESC], perPage: 25, page: 1) {
        ...
        role        voiceActors {
        id          name {
        first            middle            last            full            native            userPreferred          }
        image {
        large            medium          }
        languageV2        }
        node {
    ...
    externalLinks {
        id      url      site      type      icon      color      language    }  }  Page(page: 1) {    $standardPageInformation    mediaList(isFollowing: true, sort: [STATUS], mediaId: $id) {
        id      status      score(format: POINT_100)      progress      progressVolumes      user {
        id        name        avatar {
        large          medium        }
        }
