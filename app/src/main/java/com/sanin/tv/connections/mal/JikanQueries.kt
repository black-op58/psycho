...
return tryWithSuspend {            rateLimitedGet("$apiUrl/seasons/upcoming?page=$page&limit=$limit")                .parsed<JikanSearchResponse>()        }}
suspend
fun getSeason(        year: Int,        season: String,        page: Int = 1,        limit: Int = 15,    ): JikanSearchResponse? {
return tryWithSuspend {            rateLimitedGet("$apiUrl/seasons/$year/$season?page=$page&limit=$limit")                .parsed<JikanSearchResponse>()        }}
suspend
...
return tryWithSuspend {            rateLimitedGet("$apiUrl/users?q=$encodedQuery&page=$page")                .parsed<JikanUserSearchResponse>()        }}
suspend
fun getUserProfile(username: String): JikanUserRef? {
    val encodedUsername = URLEncoder.encode(username, "UTF-8")
return tryWithSuspend {            rateLimitedGet("$apiUrl/users/$encodedUsername")                .parsed<JikanUserProfileResponse>()                .data        }}
suspend
fun getCharacterFull(malId: Int): JikanCharacterFullData? {
return tryWithSuspend {            rateLimitedGet("$apiUrl/characters/$malId/full")                .parsed<JikanCharacterFullResponse>()                .data        }}
suspend
fun getPersonFull(malId: Int): JikanPersonFullData? {
return tryWithSuspend {            rateLimitedGet("$apiUrl/people/$malId/full")                .parsed<JikanPersonFullResponse>()                .data        }}
suspend
