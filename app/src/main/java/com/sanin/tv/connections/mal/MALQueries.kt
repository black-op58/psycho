...
return tryWithSuspend {
        executeRequest {
        client.get(                    "$apiUrl/anime/suggestions?limit=$limit&fields=$rankingFields",                    headers                )            }.parsed<MalRankingResponse>()
        }
}
...
return tryWithSuspend {
        executeRequest {
        client.get(                    "$apiUrl/users/@me/animelist?fields=$sequelListFields&status=completed&sort=list_updated_at&limit=$limit&nsfw=1",                    headers                )            }.parsed<MalListResponse>()        }}
suspend
