...
} else {                    prefs.edit().putString(UpcomingWidget.PREF_SERIALIZED_MEDIA, "").apply()
...
return try {
    val gson = GsonBuilder()                .registerTypeAdapter(SAnime::class.java, InstanceCreator<SAnime> {                    
        S
        } catch (e: Exception) {            Logger.log("Error serializing media: $e")
        Logger.log(e)            null
        }
    }
...
return try {
    val gson = GsonBuilder()                .registerTypeAdapter(SAnime::class.java, InstanceCreator<SAnime> {                    
        S
        } catch (e: Exception) {            Logger.log("Error deserializing media: $e")
        Logger.log(e)            null
        }
    }
