...
else -> dp[i][j] = minOf(                            dp[i - 1][j - 1] + if (s1[i - 1] == s2[j - 1]) 0 else 1,                            dp[i - 1][j] + 1,                            dp[i][j - 1] + 1                        )                    }                }            }
...
if (distance < minDistance) {                    minDistance = distance                    closestString = str                    closestIndex = index                }            }
...
return listOf(list[closestIndex]) + list.subList(0, closestIndex) + list.subList(                closestIndex + 1,                list.size            )        }
...
if (closestIndex == -1) {                Logger.log("No closest show found for $target")
return shows // Return original list if no closest show found            }            Logger.log("Closest show found for $target is ${closestShowAndIndex.first.name}")
...
if (distance < minDistance) {                    minDistance = distance                    closestShow = show                    closestIndex = index                }            }
