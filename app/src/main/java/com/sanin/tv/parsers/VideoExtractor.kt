...
import java.io.Serializable/** * Used to extract videos from a specific video host, * * A new instance is created for every embeds/iframes of that Episode * **/
abstract class VideoExtractor : Serializable {    
abstract val server: VideoServer
var videos: List<Video> = listOf()    
var subtitles: List<Subtitle> = listOf()    
var audioTracks: List<Track> = listOf()    /**     * Extracts videos & subtitles from the `embed`     *     * returns a container containing both videos & subtitles (optional)     * **/    
abstract suspend 
...
val audioTracks: List<Track> = listOf(),) : Serializable/** * The Class which contains all the information about a Video * **/
data class Video(    /**     * Will represent quality to user in form of `"${quality}p"` (1080p)     *     * If quality is null, shows "Unknown Quality"     *     * If isM3U8 is true, shows "Multi Quality"     * **/    
val quality: Int?,    /**     * Mime type / Format of the video,     *     * If not a "CONTAINER" format, the app show video as a "Multi Quality" Link     * "CONTAINER" formats are Mp4 & Mkv     * **/    
