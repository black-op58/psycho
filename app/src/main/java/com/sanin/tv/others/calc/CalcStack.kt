...
when {                expression[i] == ' ' -> i++                expression[i].isDigit() || expression[i] == '.' -> {
    var value = 0.0
var isDecimal = false
var decimalFactor = 0.1
while (i < expression.length && (expression[i].isDigit() || expression[i] == '.' && !isDecimal)) {
if (expression[i] == '.') {                            isDecimal = true
} else if (!isDecimal) {                            value = value * 10 + (expression[i] - '0')
} else {                            value += (expression[i] - '0') * decimalFactor                            decimalFactor *= 0.1                        }                        i++                    }                    values.push(value)                    i-- // to compensate the additional i++ in the loop
}
else -> {
while (!ops.isEmpty() && precedence(ops.peek()) >= precedence(expression[i])) {
    val val2 = values.pop()                        
val val1 = values.pop()                        
val op = ops.pop()                        values.push(applyOp(val1, val2, op))                    }                    ops.push(expression[i])                }            }            i++        }
while (!ops.isEmpty()) {
    val val2 = values.pop()            
val val1 = values.pop()            
