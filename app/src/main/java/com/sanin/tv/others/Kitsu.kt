...
try {
    val query =                        """                        query {                          lookupMapping(externalId: ${media.id}, externalSite: ANILIST_ANIME) {                            __typename                            ... on Anime {                              id                              episodes(first: 2000) {                                nodes {                                  number                                  titles {                                    canonical                                  }                                  description(locales: ["en", "en-us"])                                  thumbnail {                                    original {                                      url                                    }                                  }                                }                              }                            }                          }                        }""".trimIndent()                    
...
val animeId = searchRes.data?.firstOrNull()?.id ?: return@tryWithSuspend null                media.idKitsu = animeId                                Logger.log("Kitsu : Used REST API Method (2nd Priority)")                // Fetch Episodes with Pagination
